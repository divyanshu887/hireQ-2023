...
export default function JobDescription({ currentUser, logout }) {
  ...
  const handleModalView = (id) => {
    ...
    console.log(
      "id",
      id,
      records.filter((items) => items.id === id)
    );
    setModalData(records.filter((items) => items.id === id)[0]);
    setModalOpen(true);
  };
  ...
  return (
    <>
      {modalOpen && <Jdmodal setOpenModal={setModalOpen} jdData={modalData} />}
      <PageHeader
            ...
            {recordsAfterPagingAndSorting().map((item) => (
              <TableRow key={item.id}>
                ...
                <TableCell>
                  ...
                  <Controls.ActionButton
                    color="primary"
                    onClick={() => handleModalView(item.id)}
                  >
